...
var loadConfigurationFromAppConfig = function(onSuccess, context) {
    ...
    try {
        ...
        sap.Logger.error('Failed to load the appConfig.js file: ' + e.toString());
        if (e.message){
...
var handleOpenURL = function(errorCallback, url) {
    sap.Logger.debug('fioriclient handleOpenURL called' + url, 'FIORI_CLIENT');
    ...
    if (url && url.indexOf("x-source=com.sap.authenticator") != -1 && url.indexOf("://x-callback-url/setCredential?") != -1) {
        ...
        setTimeout(function() {
            ...
            if (userValue || userName){
                ...
                console.log("handleopenurl set webview url for otp" + window.location + "&" + credential);
...
var addBusterToURL = function(requestedURL) {
    if (context.appConfig.skiphomebuster){
        ...
        var sDeviceTokenQuery = "deviceToken=" + window.localStorage.getItem("deviceToken");
		...
		if (requestedURL.indexOf("?") != -1) {
			...
			requestedURL = preFirstQuestionMark + "?" + buster + "&" + sDeviceTokenQuery + "&" + postFirstQuestionMark;
		} else {
			if (requestedURL.indexOf("#") != -1) {
				// Need to add query before anchor
				requestedURL = requestedURL.split("#")[0] + "?" + buster + "&" + sDeviceTokenQuery + "#" + requestedURL.split("#")[1];
			} else {
				// No anchor or query so create one.
				requestedURL = requestedURL + '?' + buster + "&" + sDeviceTokenQuery;
			}
		}
                    ...
                    if (cordova.require("cordova/platform").id == "ios" && sap.AuthProxy.OAuth2.isProtectedEndpoint(retrievedFioriURL)){
                        ...
                        sap.AuthProxy.OAuth2.getToken(function(token) {
                            if (token && token.access_token) {
                                var header = {"Authorization": "Bearer " + token.access_token};
                                if (context && context.smpRegContext && context.smpRegContext.applicationConnectionId){
                                    header["X-SMP-APPCID"] = context.smpRegContext.applicationConnectionId;
                                }
